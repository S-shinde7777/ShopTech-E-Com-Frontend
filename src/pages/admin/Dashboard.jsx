    ...
    <div className="space-y-8">
      ...
      {loading ? (
        <div className="py-20 text-center flex flex-col items-center justify-center">
          <div className="w-10 h-10 border-4 border-[#5FE3CF] border-t-transparent rounded-full animate-spin"></div>
          <p className="mt-4 text-gray-400 text-sm">Loading admin dashboard statistics...</p>
        </div>
      ) : (
        <>
          {/* Stats cards Grid */}
                  ...
                  <tbody className="divide-y divide-white/5">
                    {recentOrders.map((order) => (
                      <tr key={order.id} className="text-gray-300">
                        <td className="py-4 font-bold text-[#5FE3CF]">#{order.id.slice(-6)}</td>
